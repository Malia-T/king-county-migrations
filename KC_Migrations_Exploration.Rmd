...
median_housing_2011_2020 <- inner_join(med_h_2011_2015, med_h_2016_2020, by = "CBSAA") %>% 
...
ggplot(data = pops_movers_2015, mapping = aes(x = race, y = pct_movers)) +
  geom_histogram(na.rm = TRUE, stat = "sum") +
  ggtitle("Percent of Race Moving out of Seattle-Tacoma-Bellevue MSA (2011-2015)")
```
While there are more white movers (way outnumber other classes), the percent of people in the "two or more/other" category that are moving out of Seattle-Tacoma-Bellevue vastly outnumbers the other race categories.
...
flows_housing_ <- inner_join(sea_flows_2020, median_housing_2011_2020, by = c("FULL2_NAME" = "NAME_E.x"))

data <- flows_housing_ %>% 
  inner_join(race_2010_2020, by = c("FULL2_NAME" = "CBSA"))
...
```
```{r}
# random forest with m = sqrt(p) ~ 3 trees
...
mse <- mean((pcts.train - rf.moves)^2)
rmse <- sqrt(mse)
mae <- mean(abs(pcts.test$estimate - tree.pred))
mape <- mean(abs((pcts.test$estimate - tree.pred) / pcts.test$estimate)) * 100
```
