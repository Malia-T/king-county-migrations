...
```
- Supplement with US census data manually? Also, not all race categories are reflected in the census county estimates... include all but Hispanic/Latino?
- Supplement totals with county pop data
- Remove the missing ones for now and train?
...
calc_differences <- function(data) {
  ...
  pct_diff <- sapply(race_cols, function(race) {
    (kc[[race]] - data[[race]]) / data[[race]] * 100
  })
...
train <- sample(1:nrow(pct.2019), nrow(pct.2019) * 0.8)

vars <- c("movers", "white_alone", "black_alone", "amer_ind_ak_alone", "asian_alone", "nat_haw_pac_alone", "other_alone", "two_plus")

pct.2019 <- pct.2019 %>% 
  select(all_of(vars))

pcts.train <- pct.2019[train,]
pcts.test <- pct.2019[-train,]
