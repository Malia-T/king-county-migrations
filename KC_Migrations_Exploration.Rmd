...
ACS migration data from ACS `tidycensus` package. County population, race, age, sex data from NHGIS IPUMS.
...
```{r}
set_token("pk.eyJ1IjoibWFsaWFqYyIsImEiOiJjazNxYzBkZXgwMDMwM2VxdXZtanZwM2t3In0.It8ATez8CuG6PlSeu9-SUQ")
...
movers_by_race_2015 <- sea_flows_2015 %>% 
  filter(variable == "MOVEDOUT") %>% 
  filter(RACE_label != "All races") %>% 
  select(c(RACE_label, variable, estimate))
